...
void EtudiantDialog::on_searchButton_clicked()
{
    ...
    if (searchText.isEmpty()) {
        ...
        model->setFilter(QString("name LIKE '%%1%' OR family_name LIKE '%%1%' OR cin LIKE '%%1%'").arg(searchText));
    }
}
...
{
    EtudiantService service;
    QSqlRecord record = service.getById(studentId);
    
    if (!record.isEmpty()) {
        ui->nameEdit->setText(record.value("name").toString());
        ui->familyNameEdit->setText(record.value("family_name").toString());
        ui->cinEdit->setText(record.value("cin").toString());
        ui->specialityCombo->setCurrentText(record.value("speciality").toString());
        ui->noteEdit->setText(record.value("note").toString());
        ui->genderCombo->setCurrentText(record.value("gender").toString());
        ui->phoneEdit->setText(record.value("phone").toString());
        ui->emailEdit->setText(record.value("email").toString());
        ui->birthDateEdit->setDate(QDate::fromString(record.value("birth_date").toString(), "yyyy-MM-dd"));
        
        QString photoPath = record.value("photo_path").toString();
...
void EtudiantDialog::updateGenderStats()
{
    ...
    QSqlQuery query;
    query.prepare("SELECT gender, COUNT(*) as count FROM etudiants GROUP BY gender");
    if (!query.exec()) {
...
bool EtudiantDialog::validateInputs()
{
    ...
    if (ui->familyNameEdit->text().trimmed().isEmpty()) {
        QMessageBox::warning(this, "Validation Error", "Family name is required");
        ui->familyNameEdit->setFocus();
        return false;
    }
    
    // CIN validation
    if (ui->cinEdit->text().trimmed().isEmpty()) {
        QMessageBox::warning(this, "Validation Error", "CIN is required");
        ui->cinEdit->setFocus();
        return false;
    }
    ...
    // Note validation
    bool isValid;
    double note = ui->noteEdit->text().toDouble(&isValid);
