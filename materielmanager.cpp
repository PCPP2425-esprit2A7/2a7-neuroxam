...
{
    QSqlQuery query;
    query.prepare("INSERT INTO materiels (type, etat, localisation, disponibilite, ID_CENTRE_FK) "
                  "VALUES (:type, :etat, :localisation, :disponibilite, :ID_CENTRE_FK)");
    ...
    // jointure center
    int idCentre = materiel.getLocalisation().section('#', 1).toInt();
    query.bindValue(":ID_CENTRE_FK", idCentre);
...
bool MaterielManager::updateMateriel(const Materiel& materiel)
{
    ...
    query.prepare("UPDATE materiels SET type = :type, etat = :etat, localisation = :localisation, "
                  "disponibilite = :disponibilite, ID_CENTRE_FK = :ID_CENTRE_FK WHERE id = :id");
