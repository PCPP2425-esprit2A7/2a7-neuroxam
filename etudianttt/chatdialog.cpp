...
void ChatDialog::setupUi()
{
    ...
    testConnectionButton = new QPushButton("Tester API", this);
    testConnectionButton->setStyleSheet(
        "QPushButton { background: none; border: none; color: #007bff; }"
        "QPushButton:hover { text-decoration: underline; }"
        );
    toolbarLayout->addWidget(testConnectionButton);
    ...
    connect(testConnectionButton, &QPushButton::clicked, this, &ChatDialog::testConnection);
}
...
void ChatDialog::testConnection()
{
    // Update status indicator to show we're testing
    statusLabel->setText("●");
    statusLabel->setStyleSheet("QLabel { color: orange; font-size: 14px; }");
    statusLabel->setToolTip("Test de connexion en cours...");

    // Add info message to chat
    QString html = QString(
        "<div style='margin: 10px 0; text-align: center;'>"
        "  <div style='color: blue; font-style: italic;'>Test de connexion à l'API en cours...</div>"
        "</div>"
        );
    chatDisplay->append(html);

    // Call the test API connection method
    chatbotService->testApiConnection();

    // The results will be displayed by the displayError slot
}
