    ...
    ui(new Ui::Deplome)
{
    ...
    // 🔁 Remplir la comboBoxEmploye avec tous les employés
    QSqlQuery query("SELECT ID, NOM || ' ' || PRENOM FROM EMPLOYE");
    while (query.next()) {
        int id = query.value(0).toInt();
        QString nomComplet = query.value(1).toString();
        ui->comboBoxEmploye->addItem(nomComplet, id);  // Texte = nom complet, data = ID
    }
...
void Deplome::on_btnRechercherFormation_clicked()
{
    ...
    if (query.exec() && query.next()) {
        ...
        ui->labelDuree->setText("Durée : " + QString::number(duree) + " h");
    } else {
...
{
    QString idFormation = ui->lineEditIdFormation->text().trimmed();
    int idEmploye = ui->comboBoxEmploye->currentData().toInt();
    QString nomEmploye = ui->comboBoxEmploye->currentText().trimmed();

    if (idFormation.isEmpty() || sujet.isEmpty() || nomEmploye.isEmpty()) {
        QMessageBox::warning(this, "Erreur", "Remplissez tous les champs nécessaires.");
        return;
    }

    // 🔁 Associer l'employé à la formation dans la base de données
    QSqlQuery updateQuery;
    updateQuery.prepare("UPDATE FORMATION SET ID_EMPLOYE = :idEmploye WHERE ID_FORMATION = :idFormation");
    updateQuery.bindValue(":idEmploye", idEmploye);
    updateQuery.bindValue(":idFormation", idFormation);
    if (!updateQuery.exec()) {
        QMessageBox::warning(this, "Erreur", "❌ Impossible d'associer l'employé à la formation.");
        return;
    }
    ...
    QMessageBox::information(this, "Succès", "🎉 Certificat généré, signé et associé !");
}
