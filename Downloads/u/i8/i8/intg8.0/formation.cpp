...
QSqlQueryModel* Formation::rechercher(const QString& valeur) {
    ...
    if (valeur.toInt() > 0) {
        // Recherche par ID_FORMATION si la valeur est un entier
        queryString = "SELECT * FROM FORMATION WHERE ID_FORMATION = :valeur";
        query.prepare(queryString);
        query.bindValue(":valeur", valeur);
    } else {
        // Recherche par SUJET, STATUT, NOM_FORMATEUR ou NOM_CENTRE
        queryString = "SELECT * FROM FORMATION "
                      "WHERE SUJET LIKE :valeurLike "
                      "OR STATUT LIKE :valeurLike "
                      "OR NOM_FORMATEUR LIKE :valeurLike "
                      "OR NOM_CENTRE LIKE :valeurLike";
        query.prepare(queryString);
