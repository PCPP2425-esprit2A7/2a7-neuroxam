...
{
    QSqlQuery query;
    query.prepare("SELECT nom, id FROM centre");
    ...
    ui->lineEditCentre->addItem("");
    ui->localisation->addItem("");

    while (query.next()) {
        QString name = query.value(0).toString();
        int id = query.value(1).toInt();
        QString displayText = name + "#" + QString::number(id);

        ui->lineEditCentre->addItem(displayText);
        ui->localisation->addItem(displayText);
    }
}
